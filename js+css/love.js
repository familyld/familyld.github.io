(function(window){
    ...
    Seed.prototype = {
        ...
        drawText: function() {
            ...
            ctx.fillText("Seed of Love", 23, 16);
            ctx.restore();
        },
        clear: function() {
            var ctx = this.tree.ctx, cirle = this.cirle;
    ...
    Tree = function(canvas, width, height, opt) { // 画布对象，画布的宽和长，花树的设置
        this.canvas = canvas;
        this.ctx = canvas.getContext('2d'); // 返回一个对象，该对象提供了用于在画布上绘图的方法和属性
        this.width = width;
        this.height = height;
        this.opt = opt || {};
    ...
    Tree.prototype = {
        initSeed: function() {
            ...
            // opt中没设定seed的横纵坐标时，seed坐标默认画在canvas的正中间
        ...
        addBranchs: function(branchs){
        	...
        	for (var i = 0; i < branchs.length; i++) {
                b = branchs[i];
                p1 = new Point(b[0], b[1]); // 树枝起始点横纵坐标
                p2 = new Point(b[2], b[3]); // 树枝中点横纵坐标
                p3 = new Point(b[4], b[5]); // 树枝末梢横纵坐标
                r = b[6]; // 半径
                l = b[7]; // 长度
                c = b[8]; // 下一组树枝
        ...
        canFlower: function() {
            // 如果blooms.length为0则返回False，否则返回True
