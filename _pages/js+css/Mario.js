var Mario = {
    ...
    Level: function(a, b) {
        ...
        for (var c = 0,
            ...
            for (e = 0; e < this.Height; e++) this.Map[c][e] = 0,
                this.Data[c][e] = 0,
...
Mario.Level.prototype = {
    ...
    SetBlock: function(a, b, c) { // 如不符合前四个条件，则地图的a行b列填充为c
        a < 0 || b < 0 || a >= this.Width || b >= this.Height || (this.Map[a][b] = c)
    },
    SetBlockData: function(a, b, c) { // 如不符合前四个条件，则地图的a行b列数据为c
...
Mario.BackgroundGenerator.prototype = {
    ...
    GenerateOverground: function(a) {
        for (var b = this.Distant ? 4 : 6, c = this.Distant ? 2 : 1, e = Math.floor(Math.random() * b) + c, d = Math.floor(Math.random() * b) + c, f = 0, g = 0, h = 0, i = 0, i = 2, f = 0; f < this.Width; f++) {
            ...
            for (g = 0; g < this.Height; g++) h = e < d ? e : d,
                i = e < d ? d : e,
                g < h ? this.Distant ? (i = 2, g < 2 && (i = g), a.SetBlock(f, g, 4 + i * 8)) : a.SetBlock(f, g, 5) : g === h ? (i = h === d ? 0 : 1, i += this.Distant ? 2 : 0, a.SetBlock(f, g, i)) : g === i ? (i = h === d ? 0 : 1, i += this.Distant ? 2 : 0, a.SetBlock(f, g, i + 16)) : (i = g > i ? 1 : 0, h === e && (i = 1 - i), i += this.Distant ? 2 : 0, a.SetBlock(f, g, i + 8))
        }
    },
...
Mario.BackgroundRenderer.prototype.Draw = function(a, b) {
    for (var c = b.X / this.Distance,
            e = 0,
            d = 0,
            f = null,
            f = null,
            g = (c + this.Width) / 32 | 0, e = c / 32 | 0; e <= g; e++)
        for (d = 0; d < this.TilesY; d++) f = this.Level.GetBlock(e, d) & 255,
...
Mario.ImprovedNoise.prototype = {
    Shuffle: function() {
        ...
        for (b = 0; b < 256; b++) c = (Math.random() * 255 | 0) + b,
            e = a[b],
            a[b] = a[c],
            a[c] = e,
...
Mario.Character = function() {
    ...
    this.CanShoot = !1; // 能不能开枪，只有吃了花才可以
    this.Width = 4;
    this.Height = 24;
    this.World = null;
...
Mario.Character.prototype.SetLarge = function(a, b) {
    // 若传入a为False，b为False，则a，b=0，False，人物图像是小马里奥
    // 若传入a为True，b为False，则a，b=True，False不变，人物图像是大马里奥
    // 只要b为True，则a，b=True，人物图像是无敌马里奥
    b && (a = !0); // b为true则a为true
    a || (b = !1); // a为false则b为false
...
Mario.Character.prototype.Blink = function(a) {
    ...
    // 如果this.Large为false，则人物图像依然是smallMario，否则如果this.Fire为true，则
    // 人物图像为fireMario，否则为Mario
...
Mario.Character.prototype.Move = function() {
    if (this.WinTime > 0) this.WinTime++, // 通关动作控制
        // alert('WinTime > 0:'+this.WinTime), // WinTime到67时画面转至全局地图
        this.Ya = this.Xa = 0;
    else if (this.DeathTime > 0) this.DeathTime++, // 失败动作控制
        this.DeathTime < 11 ? this.Ya = this.Xa = 0 : this.DeathTime === 11 ? this.Ya = -15 : this.Ya += 2,
        this.X += this.Xa,
        this.Y += this.Ya;
    else if (this.PowerUpTime !== 0) { // 吃了蘑菇
        // alert('PowerUpTime>0:'+this.PowerUpTime);
        ...
        if (Enjine.KeyboardInput.IsKeyDown(Enjine.Keys.S) || this.JumpTime < 0 && !this.OnGround && !this.Sliding)
            if (this.JumpTime < 0) this.Xa = this.XJumpSpeed,
        ...
        this.X > this.World.Level.ExitX * 16 && this.Win(); // 到达关卡终点，触发Win函数
        if (this.X > this.World.Level.Width * 16) this.X = this.World.Level.Width * 16,
            this.Xa = 0;
        this.Ya *= 0.85;
        this.Xa *= this.OnGround ? this.GroundInertia : this.AirInertia;
        this.OnGround || (this.Ya += 3);
        if (this.Carried !== null && (this.Carried.X *= this.X + this.Facing * 8, this.Carried.Y *= this.Y - 2, !Enjine.KeyboardInput.IsKeyDown(Enjine.Keys.A))) this.Carried.Release(this),
            this.Carried = null
    }
};
...
Mario.Character.prototype.SubMove = function(a, b) {
    ...
    if (a < 0) this.Sliding = !0,
        ...
        this.IsBlocking(this.X + a - this.Width, this.Y + b, a, b) ? c = !0 : this.Sliding = !1;
    if (c) {
        if (a < 0) this.X = ((this.X - this.Width) / 16 | 0) * 16 + this.Width,
            this.Xa = 0;
        if (a > 0) this.X = ((this.X + this.Width) / 16 + 1 | 0) * 16 - this.Width - 1,
            this.Xa = 0;
        if (b < 0) this.Y = ((this.Y - this.Height) / 16 | 0) * 16 + this.Height,
            this.Ya = this.JumpTime = 0;
        if (b > 0) this.Y = ((this.Y - 1) / 16 + 1 | 0) * 16 - 1,
            this.OnGround = !0;
        return !1
    } else return this.X += a,
        this.Y += b, !0
};
...
Mario.Character.prototype.Stomp = function(a) {
    ...
    if (!(this.DeathTime > 0 || this.World.Paused))
        if (b = a.Y - a.Height / 2, this.SubMove(0, b - this.Y), a instanceof Mario.Enemy || a instanceof Mario.BulletBill) Enjine.Resources.PlaySound("kick"),
            this.XJumpSpeed = 0,
            this.YJumpSpeed = -1.9,
            this.JumpTime = 8,
            this.Ya = this.JumpTime * this.YJumpSpeed,
...
Mario.Character.prototype.GetHurt = function() {
    if (!(this.DeathTime > 0 || this.World.Paused) && !(this.InvulnerableTime > 0))
        ...
        // 如果没有变大（取得爱心）则触发Die，否则第一次遇到危险时告知“爱会让你不可阻挡”
        this.Large ? (love==1?(alert("Love makes you unstoppable! Enemies can't hurt you anymore!"),
            love=2):love) : this.Die()
};
// 赢下一关的时候触发
Mario.Character.prototype.Win = function() {
    // alert('Win:'+trick);
    // document.getElementById("instruction").innerHTML = "祝贺你！";
    if (trick == 1) document.getElementById("seed").click(),trick=2;
    else if (trick == 2) document.getElementById("branch").click(),trick=3;
    else if (trick == 3) document.getElementById("flower").click(),trick=4;
    else if (trick == 4) document.getElementById("flower2").click(),trick=5;
    if (victory == 1) new Mario.WinState().Enter();
    // alert('Win:'+trick);
...
Mario.Character.prototype.GetFlower = function() {
    // if (!(this.DeathTime > 0 && this.World.Paused)) this.Fire ? (this.GetCoin(),
...
Mario.Character.prototype.GetMushroom = function() {
    if (!(this.DeathTime > 0 && this.World.Paused)) this.Large ? (this.GetCoin(),
        Enjine.Resources.PlaySound("coin")) : (this.World.Paused = !0,
        this.PowerUpTime = 18,
        Enjine.Resources.PlaySound("powerup"),
        this.SetLarge(!0, !0), // 既变大又能开火
        love==0?love:love=1)
};
...
Mario.LevelRenderer.prototype.DrawStatic = function(a, b) {
    for (var c = 0,
            ...
            d = 0,
            d = null,
            f = (b.X + this.Width) / 16 | 0, c = b.X / 16 | 0; c < f + 1; c++)
        for (e = 0; e < this.TilesY; e++) d = this.Level.GetBlock(c, e) & 255,
            (Mario.Tile.Behaviors[d] & Mario.Tile.Animated) === 0 && (d = this.Background[d % 16][d / 16 | 0], a.drawImage(Enjine.Resources.Images.map, d.X, d.Y, d.Width, d.Height, (c << 4) - b.X | 0, e << 4 | 0, d.Width, d.Height))
};
Mario.LevelRenderer.prototype.DrawDynamic = function(a, b) {
    for (var c = 0,
            ...
            g = 0,
            d = null,
            c = b.X / 16 | 0; c <= (b.X + this.Width) / 16 | 0; c++)
...
Mario.LevelRenderer.prototype.DrawExit0 = function(a, b, c) {
    for (var e = 0,
            e = 0,
            d = null,
            e = this.Level.ExitY - 8; e < this.Level.ExitY; e++) d = this.Background[12][e === this.Level.ExitY - 8 ? 4 : 5],
...
Mario.LevelRenderer.prototype.DrawExit1 = function(a, b) {
    for (var c = 0,
            e = null,
            c = this.Level.ExitY - 8; c < this.Level.ExitY; c++) e = this.Background[13][c === this.Level.ExitY - 8 ? 4 : 5],
...
Mario.LevelGenerator = function(a, b) {
    ...
    this.Height = b;
    this.Odds = [];
    this.Type = this.Difficulty = this.TotalOdds = 0 //修改这里会减少关卡数量
};
Mario.LevelGenerator.prototype = {
    CreateLevel: function(a, b) { // 该函数在进入关卡时触发，也即按下s的时候
        var c = 0,
            e = 0,
            d = c = 0,
            f = 0,
        ...
        for (c = 0; c < this.Odds.length; c++) this.Odds[c] < 0 && (this.Odds[c] = 0),
    ...
    AddEnemyLine: function(a, b, c, e) {
        for (var d = 0,
                ...
                d = b; d < c; d++)
            // 这里的随机数乘的数值增大会减少敌人
            if ((Math.random() * 100 | 0) < this.Difficulty + 1)
                f = Math.random() * 4 | 0,
                this.Difficulty < 1 ? f = Mario.Enemy.Goomba : this.Difficulty < 3 && (f = Math.random() * 3 | 0),
                // this.Difficulty < 1 ? f = Mario.Enemy.Goomba,
                a.SetSpriteTemplate(d, e, new Mario.SpriteTemplate(f, (Math.random() * 100 | 0) < this.Difficulty))
    },
    BuildTubes: function(a, b, c) {
        ...
        for (h = b; h < b + e; h++) {
            ...
            for (i = 0; i < this.Height; i++)
                ...
                else if ((h === f || h === f + 1) && i >= g) j = 10 + h - f,
    ...
    Blockify: function(a, b, c, e) {
        for (var d = 0,
                f = [], g = 0, h = 0, i = 0, j = 0, k = g = 0, l = 0, g = 0; g < 2; g++) f[g] = [];
...
Mario.SpriteTemplate.prototype = {
    Spawn: function(a, b, c, e) {
        if (!this.IsDead) this.Sprite = this.Type === Mario.Enemy.Flower ? new Mario.FlowerEnemy(a, b * 16 + 15, c * 16 + 24) : new Mario.Enemy(a, b * 16 + 8, c * 16 + 15, e, this.Type, this.Winged),
    ...
    if (this.DeadTime === 0) {
        ...
        if (a > -this.Width * 2 - 4 && a < this.Width * 2 + 4 && b > -this.Height && b < Mario.MarioCharacter.Height)
            if (this.Type !== Mario.Enemy.Spiky && Mario.MarioCharacter.Ya > 0 && b <= 0 && (!Mario.MarioCharacter.OnGround || !Mario.MarioCharacter.WasOnGround))
                if (Mario.MarioCharacter.Stomp(this), this.Winged) this.Winged = !1,
                    this.Ya = 0;
                else {
                    this.YPicO = 7;
                    this.PicHeight = 8;
...
Mario.Enemy.prototype.Draw = function(a, b) {
    var c = 0,
        e = 0;
    if (this.Winged && (c = (this.XOld + (this.X - this.XOld) * this.Delta | 0) - this.XPicO, e = (this.YOld + (this.Y - this.YOld) * this.Delta | 0) - this.YPicO, this.Type !== Mario.Enemy.RedKoopa && this.Type !== Mario.Enemy.GreenKoopa)) this.XFlip = !this.XFlip,
        a.save(),
...
Mario.Mushroom.prototype.BumpCheck = function(a, b) {
    if (this.X + this.Width > a * 16 && this.X - this.Width < a * 16 - 16 && b === (b - 1) / 16 | 0) this.Facing = -Mario.MarioCharacter.Facing,
        this.Ya = -10
};
...
Mario.BulletBill.prototype.ShellCollideCheck = function(a) {
    ...
    var b = a.X - this.X,
        c = a.Y - this.Y;
    if (b > -16 && b < 16 && c > -this.Height && c < a.Height) return Enjine.Resources.PlaySound("kick"),
        this.Dead = !0,
        this.Xa = 0,
        this.Ya = 1,
        this.DeadTime = 100, !0;
...
Mario.Shell.prototype.Move = function() {
    ...
    if (this.Carried) this.World.CheckShellCollide(this);
        ...
        if (!this.SubMove(this.Xa, 0)) Enjine.Resources.PlaySound("bump"),
            this.Facing = -this.Facing;
        this.OnGround = !1;
        this.SubMove(0, this.Ya);
        this.Ya *= 0.85;
...
Mario.Shell.prototype.BumpCheck = function(a, b) {
    if (this.X + this.Width > a * 16 && this.X - this.Width < a * 16 + 16 && b === ((this.Y - 1) / 16 | 0)) this.Facing = -Mario.MarioCharacter.Facing,
        this.Ya = -10
};
...
Mario.TitleState.prototype.Update = function(a) {
    this.bounce += a * 2;
    this.logoY = 40 + Math.sin(this.bounce) * 10;
    this.camera.X += a * 25;
...
//1. Enter; 2. CheckForChange; 3. Update; 4. Draw(此时画布黑了)； 5. CheckForChange; 6. Exit(此时出现开始画面)
Mario.LoadingState.prototype.Enter = function() {
    // alert("LoadingState:Enter");
    for (var a = 0,
            a = 0; a < 15; a++) this.Images[a] = {};
    // 每幅图的名字
...
Mario.LoadingState.prototype.Exit = function() {
    // alert("LoadingState:Exit");
    delete this.Images
};
Mario.LoadingState.prototype.Update = function(a) {
    // alert("LoadingState:Update");
    if (!this.ImagesLoaded) {
        this.ImagesLoaded = !0;
        for (var b = 0,
                b = 0; b < this.Images.length; b++)
    ...
    if (this.ScreenColor > 255) this.ScreenColor = 255,
        this.ColorDirection = -1;
    else if (this.ScreenColor < 0) this.ScreenColor = 0,
        this.ColorDirection = 1
};
Mario.LoadingState.prototype.Draw = function(a) {
    // alert("LoadingState:Draw");
...
Mario.LoadingState.prototype.CheckForChange = function(a) {
    // alert("LoadingState:CheckForChange");
    if (this.ImagesLoaded) Mario.GlobalMapState = new Mario.MapState,
...
Mario.MapState.prototype.GenerateLevel = function() {
    ...
    for (var h = Math.random() * 512, i = Math.random() * 512, j = Math.random() * 512, k = Math.random() * 512, a = 0; a < 21; a++) {
        this.Level[a] = [];
        this.Data[a] = [];
        for (b = 0; b < 16; b++) c = d.PerlinNoise(a * 10 + h, b * 10 + i),
    ...
    for (j = c = j = 0; j < 100 && c < 12; j++)
        if (a = (Math.random() * (20 / 3 | 0) | 0) * 3 + 2, b = (Math.random() * 5 | 0) * 3 + 1, this.Level[a][b] === Mario.MapTile.Grass) a < d && (d = a, f = b),
...
Mario.MapState.prototype.DrawRoad = function(a, b, c, e) {
    ...
    Math.random() > 0.5 && (d = !0);
    if (d) {
        for (; a > c;) this.Data[a][b] = 0,
            this.Level[a--][b] = Mario.MapTile.Road;
        for (; a < c;) this.Data[a][b] = 0,
            this.Level[a++][b] = Mario.MapTile.Road
    }
    for (; b > e;) this.Data[a][b] = 0,
        this.Level[a][b--] = Mario.MapTile.Road;
    for (; b < e;) this.Data[a][b] = 0,
        this.Level[a][b++] = Mario.MapTile.Road;
    if (!d) {
...
Mario.MapState.prototype.Travel = function(a, b, c, e) {
    if (!(this.Level[a][b] !== Mario.MapTile.Road && this.Level[a][b] !== Mario.MapTile.Level)) {
        ...
        if (this.Level[a][b] === Mario.MapTile.Level)
            ...
            else if (e > 0 && (this.Data[a][b] = -1, e > this.Farthest)) this.Farthest = e,
            this.XFarthestCap = a,
            this.YFarthestCap = b;
...
Mario.MapState.prototype.RenderStatic = function() {
    ...
    for (var a = 0,
            ...
            a = g = 0; a < 20; a++)
        for (b = 0; b < 15; b++)
            if (this.MapContext.drawImage(i, (this.WorldNumber / 4 | 0) * 16, 0, 16, 16, a * 16, b * 16, 16, 16), this.Level[a][b] === Mario.MapTile.Level) g = this.Data[a][b],
            ...
            else if (this.Level[a][b] === Mario.MapTile.Road) c = this.IsRoad(a - 1, b) ? 1 : 0,
        ...
        for (g = 0; g < 2; g++)
            for (h = 0; h < 2; h++) c = this.IsWater(a * 2 + (g - 1), b * 2 + (h - 1)) ? 0 : 1,
    ...
    if (this.WorldNumber !== 8) {
        ...
        if (this.MoveTime > 0) this.MoveTime--;
            ...
            if (this.CanEnterLevel && Enjine.KeyboardInput.IsKeyDown(Enjine.Keys.S) && this.Level[b][c] === Mario.MapTile.Level && this.Data[b][c] !== -11 && this.Level[b][c] === Mario.MapTile.Level && this.Data[b][c] !== 0 && this.Data[b][c] > -10) {
                ...
                this.Data[b][c] < 0 ? (this.Data[b][c] === -2 ? (Mario.MarioCharacter.LevelString += "X", e += 2, victory=1) : this.Data[b][c] === -1 ? Mario.MarioCharacter.LevelString += "?" : (Mario.MarioCharacter.LevelString += "#", e += 1), d = Mario.LevelType.Castle) : Mario.MarioCharacter.LevelString += this.Data[b][c];
                this.EnterLevel = !0;
...
Mario.MapState.prototype.TryWalking = function(a, b) {
    var c = this.XMario / 16 | 0,
        ...
        d = c + a,
        f = e + b;
    if ((this.Level[d][f] === Mario.MapTile.Road || this.Level[d][f] === Mario.MapTile.Level) && !(this.Level[d][f] === Mario.MapTile.Road && this.Data[d][f] !== 0 && this.Data[c][e] !== 0 && this.Data[c][e] > -10)) this.XMarioA = a * 8,
        this.YMarioA = b * 8,
    ...
    if (this.WorldNumber !== 8) {
        ...
        for (c = 0; c <= 15; c++)
            for (b = 20; b >= 0; b--)
                ...
                } else if (this.Level[b][c] === Mario.MapTile.Decoration) this.DecoSprite.X = b * 16,
            this.DecoSprite.Y = c * 16,
            this.DecoSprite.Draw(a, this.camera);
        else if (this.Level[b][c] === Mario.MapTile.Level && this.Data[b][c] === -2) this.HelpSprite.X = b * 16 + 16,
...
Mario.LevelState.prototype.Update = function(a) {
    ...
    for (b = this.FireballsOnScreen = 0; b < this.Sprites.Objects.length; b++) d = this.Sprites.Objects[b],
    ...
    if (this.Paused)
        ...
        for (c = (this.Camera.X / 16 | 0) - 1; c <= ((this.Camera.X + this.Layer.Width) / 16 | 0) + 1; c++)
            for (e = (this.Camera.Y / 16 | 0) - 1; e <= ((this.Camera.Y + this.Layer.Height) / 16 | 0) + 1; e++) {
                ...
                if (f !== null) f.LastVisibleTick !== this.Tick - 1 && (f.Sprite === null || !this.Sprites.Contains(f.Sprite)) && f.Spawn(this, c, e, d),
        ...
        for (b = 0; b < this.ShellsToCheck.length; b++)
            for (c = 0; c < this.Sprites.Objects.length; c++)
                if (this.Sprites.Objects[c] !== this.ShellsToCheck[b] && !this.ShellsToCheck[b].Dead && this.Sprites.Objects[c].ShellCollideCheck(this.ShellsToCheck[b]) && Mario.MarioCharacter.Carried === this.ShellsToCheck[b] && !this.ShellsToCheck[b].Dead) Mario.MarioCharacter.Carried = null,
...
Mario.LevelState.prototype.Draw = function(a) {
    ...
    if (Mario.MarioCharacter.WinTime > 0) {
        ...
        b = b * b * 0.2;
        if (b > 900) Mario.GlobalMapState.LevelWon(),
            this.GotoMapState = !0;
...
Mario.LevelState.prototype.RenderBlackout = function(a, b, c, e) {
    if (!(e > 320)) {
        for (var d = [], f = [], g = 0, g = 0; g < 16; g++) d[g] = b + Math.cos(g * Math.PI / 15) * e | 0,
        ...
        a.closePath();
        a.fill();
        for (g = 0; g < 16; g++) d[g] = b - Math.cos(g * Math.PI / 15) * e | 0,
            f[g] = c - Math.sin(g * Math.PI / 15) * e | 0;
        f[15] += 5;
        d[16] = 320;
        f[16] = c;
